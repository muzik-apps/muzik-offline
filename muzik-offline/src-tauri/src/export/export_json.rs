...
#[tauri::command]
pub async fn export_songs_as_json(
    app: tauri::AppHandle,
    ...
    let json_songs = export_songs
        .iter()
        .map(|song| extract_song(song, &fields_to_include))
        .collect::<Vec<String>>()
        .join(",\n");
...
fn extract_song(song: &ExportSong, fields_to_include: &Vec<String>) -> String {
    ...
    for field in fields_to_include {
        match field.as_str() {
            ...
            "path" => json.push_str(&format!(
                "\t\t\t\"path\": \"{}\",\n",
                convert_single_to_double_backward_slash_on_path(&song.path)
            )),
            "date_recorded" => json.push_str(&format!(
                "\t\t\t\"date_recorded\": \"{}\",\n",
                song.date_recorded
            )),
            "date_released" => json.push_str(&format!(
                "\t\t\t\"date_released\": \"{}\",\n",
                song.date_released
            )),
            "file_size" => json.push_str(&format!("\t\t\t\"file_size\": {},\n", song.file_size)),
            "file_type" => {
                json.push_str(&format!("\t\t\t\"file_type\": \"{}\",\n", song.file_type))
            }
            "overall_bit_rate" => json.push_str(&format!(
                "\t\t\t\"overall_bit_rate\": {},\n",
                song.overall_bit_rate
            )),
            "audio_bit_rate" => json.push_str(&format!(
                "\t\t\t\"audio_bit_rate\": {},\n",
                song.audio_bit_rate
            )),
            "sample_rate" => {
