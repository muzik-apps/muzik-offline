...
use crate::{
    components::song::ExportSong,
    database::{db_api::get_songs_in_tree, db_manager::DbManager},
    utils::general_utils::convert_single_to_double_backward_slash_on_path,
};

#[tauri::command]
pub async fn export_songs_as_xml(
    app: tauri::AppHandle,
    ...
    let oldest_song = songs
        .iter()
        .min_by_key(|song| song.year)
        .map(|song| song.name.clone());
    let youngest_song = songs
        .iter()
        .max_by_key(|song| song.year)
        .map(|song| song.name.clone());

    let longest_song = songs
        .iter()
        .max_by_key(|song| song.duration_seconds)
        .map(|song| song.name.clone());
    let shortest_song = songs
        .iter()
        .min_by_key(|song| song.duration_seconds)
        .map(|song| song.name.clone());

    let largest_file = songs
        .iter()
        .max_by_key(|song| song.file_size)
        .map(|song| song.name.clone());
    let smallest_file = songs
        .iter()
        .min_by_key(|song| song.file_size)
        .map(|song| song.name.clone());
    ...
    let export_songs: Vec<ExportSong> = songs
        .iter()
        .map(|song| ExportSong {
    ...
    let songs_xml = export_songs
        .iter()
        .map(|song| extract_fields_for_song(&song, &fields_to_include))
        .collect::<String>();
...
{}</songs>",
        ...
        num_unique_albums,
        num_unique_genres,
        oldest_song.unwrap_or("".to_string()),
        youngest_song.unwrap_or("".to_string()),
        longest_song.unwrap_or("".to_string()),
        shortest_song.unwrap_or("".to_string()),
        largest_file.unwrap_or("".to_string()),
        smallest_file.unwrap_or("".to_string()),
...
fn extract_fields_for_song(song: &ExportSong, fields_to_include: &Vec<String>) -> String {
    ...
    for field in fields_to_include.iter() {
        match field.as_str() {
            ...
            "duration" => {
                song_str.push_str(&format!("\t\t<duration>{}</duration>\n", song.duration))
            }
            "path" => song_str.push_str(&format!(
                "\t\t<path>{}</path>\n",
                convert_single_to_double_backward_slash_on_path(&song.path)
            )),
            "date_recorded" => song_str.push_str(&format!(
                "\t\t<dateRecorded>{}</dateRecorded>\n",
                song.date_recorded
            )),
            "date_released" => song_str.push_str(&format!(
                "\t\t<dateReleased>{}</dateReleased>\n",
                song.date_released
            )),
            "file_size" => {
                song_str.push_str(&format!("\t\t<fileSize>{}</fileSize>\n", song.file_size))
            }
            "file_type" => {
                song_str.push_str(&format!("\t\t<fileType>{}</fileType>\n", song.file_type))
            }
            "overall_bit_rate" => song_str.push_str(&format!(
                "\t\t<overallBitRate>{}</overallBitRate>\n",
                song.overall_bit_rate
            )),
            "audio_bit_rate" => song_str.push_str(&format!(
                "\t\t<audioBitRate>{}</audioBitRate>\n",
                song.audio_bit_rate
            )),
            "sample_rate" => song_str.push_str(&format!(
                "\t\t<sampleRate>{}</sampleRate>\n",
                song.sample_rate
            )),
            "bit_depth" => {
                song_str.push_str(&format!("\t\t<bitDepth>{}</bitDepth>\n", song.bit_depth))
            }
            "channels" => {
                song_str.push_str(&format!("\t\t<channels>{}</channels>\n", song.channels))
            }
            _ => (),
        }
    }
